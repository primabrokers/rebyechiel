...
export function PreviewBanner() {
  ...
  const role = demoRole();
  return (
    <div className="sticky top-0 z-40 bg-warn text-white px-4 py-2 flex items-center justify-between gap-3">
      ...
      <div className="flex items-center gap-3 flex-none">
        <a href={role === 'rabbi' ? '/?preview=member' : '/rabbi?preview=rabbi'}
          className="text-[12px] font-bold underline underline-offset-2">
          Switch
        </a>
        <button onClick={exitDemo} className="text-[12px] font-bold underline underline-offset-2">Exit</button>
      </div>
    </div>
  );
}
