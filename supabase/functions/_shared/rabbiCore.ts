...
import { computeEta, localParts } from "./rabbiEta.ts";
...
export async function expandSlots(
  ...
  const horizon = new Date(Date.now() + 21 * 86_400_000).toISOString();
  const [releasesQ, bookingsQ, blocksQ] = await Promise.all([
  ...
  for (const r of releases) {
    const dur = (r.duration_minutes as number) * 60_000;
    for (let s = Date.parse(r.starts_at); s + dur <= Date.parse(r.ends_at); s += dur) {
      const e = s + dur;
      ...
      const endMin = startMin + (r.duration_minutes as number);
      const blocked = blocks.some((bl) => {
        if (bl.weekday !== wd) return false;
        const [bh, bm] = String(bl.start_time).split(":").map(Number);
        const [eh, em] = String(bl.end_time).split(":").map(Number);
        return startMin < eh * 60 + em && endMin > bh * 60 + bm;
      });
      if (blocked) continue;
      out.push({
        releaseId: r.id, slotType, startsAt: new Date(s).toISOString(),
        endsAt: new Date(e).toISOString(), location: r.location ?? null,
      });
...
export async function createBooking(
  ...
  const { data, error } = await admin.from("rabbi_bookings").insert({
    ...
    contact_name: input.contactName ?? null,
    contact_phone: input.contactPhone ?? null,
    slot_release_id: input.slot.releaseId,
